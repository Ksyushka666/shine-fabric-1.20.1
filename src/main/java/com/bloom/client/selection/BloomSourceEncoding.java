...
public final class BloomSourceEncoding {
	...
	public static int encodePackedLight(int packedLight, double sourceStrength) {
		double clamped = clampSourceStrength(sourceStrength);
		int low6;
		int mode;
	...
	private static double clampSourceStrength(double sourceStrength) {
		if (!Double.isFinite(sourceStrength)) return BloomConfig.MIN_SOURCE_STRENGTH;
		return Math.max(BloomConfig.MIN_SOURCE_STRENGTH, Math.min(BloomConfig.MAX_SOURCE_STRENGTH, sourceStrength));
	}
